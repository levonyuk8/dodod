...
export class ThreeHelperService {
  ...
  private textureF = new THREE.TextureLoader().load('img/qwe.jpg');
...
// Использование
  ...
  private createPlanki(w: number, h: number, d: number, thickness: number, srK: number, data: Wardrobe) {
      ...
      res.push(
        {
          ...
          element: this.createCube(plankaVCenterW, data.SR_H_PLANKA_VERH, thickness, true),
    ...
    if (data?.SR_H_PLANKA_BOK_PRAV) {
      res.push(
        {
          name: 'правая',
          element: this.createCube(thickness, plankaBokH, plankaBokD, true),
    ...
    if (data?.SR_PLANKA_BOK_CHENTR) {
      res.push(
        {
          name: 'верхняя',
          element: this.createCube(plankaBokCenterW, thickness, plankaBokD, true),
  ...
  private addAntresoli() {
    ...
    group.add(...meshes);
    this.scene.add(group);
    const antrSections = this.createAntrSections(data.srL, data.srH, data.srG, this.depth, data.srK, scheme, data);
    if (antrSections.length === 0) return;
    antrSections.forEach((section: any, index) => {
      section.element.position.set(section.position.x, section.position.y, section.position.z);
      meshes.push(section.element)
    });
  }
  ...
  createAntresoli(w: number, h: number, d: number, thickness: number, srK: number, data: Wardrobe) {
    ...
    for (let i = 0; i < srK; i++) {
      ...
      const door = {
        ...
        section: i,
        name: 'Entresol',
        element: this.createCube(doorW, doorH, thickness, true),
        position: {
  ...
  private transparentDoor(child: any) {
    const textureMaterial = new THREE.MeshBasicMaterial({map: this.textureF, opacity: .6, transparent: true});
  ...
  private createCube(w: number, h: number, d: number, isFacade = false, opacity = 1, test = true, color = 0xffffff) {
    const mapTexture = isFacade ? this.textureF : this.texture;
    const textureMaterial = test ? new THREE.MeshBasicMaterial({map: mapTexture}) :
  ...
  createAntrSections(w: number, h: number, d: number, thickness: number, srK: number, scheme: Block[], data: any) {
    ...
    let doorH = data.SR_H_antr - thickness - thickness / 2;
    ...
    for (let i = 1; i < srK; i++) {
      const sect = {
        ...
        section: i,
        name: 'section',
        element: this.createCube(thickness, doorH - 8, d),
        position: {
          x: (-w / 2) + doorW * i,
          y: (h - doorH) / 2 - thickness, //thickness - (this.plinth / 2),
          z: 0
        },
      }
      listS.push(sect);
  ...
  createYaschikVneshnie(w: number, h: number, d: number, thickness: number, scheme: any[], data: any) {
    ...
    for (let iS = 0; iS < scheme.length; iS++) {
      ...
      for (let j = 0; j < section.SR_yaschiki_vneshnie_kol; j++) {
        const sect = {
          name: 'sect',
          element: this.createCube(wYV + thickness, this.yvH, thickness, true),
          position: {
            x: startX + section.startPos * ((wYV + thickness) / 2),
  ...
  createDoors(w: number, h: number, d: number, thickness: number, srK: number, scheme: any[], data: Wardrobe) {
    ...
    for (let i = 0; i < srK; i++) {
      ...
      let doorH = data.SR_niz_dveri.toString() === '0' ? h - this.plinth : h - 10;
      let allYVH = 0;
      ...
      // doorHandle.position.y = -h / 2 + 1000;
      // doorHandle.position.x = doorW / 2 - 45 - 10;
      // // doorHandle.position.set(door.position.x,  -h / 2 + 1000 , door.position.z)
      // door.element.add(doorHandle);
...
//   size
  ...
  createDimensions(offsetX = 200, offsetY = 200) {
    ...
    // Основная линия
    const mainPoints = [start, end, end2, start2];
    const mainGeometry = new THREE.BufferGeometry().setFromPoints(mainPoints);
    const mainMaterial = new THREE.LineBasicMaterial({
      color: 0x757575,
      linewidth: 3
    ...
    const mainLine = new THREE.Line(mainGeometry, mainMaterial);
    this.dimensionsGroup.add(mainLine);

    const dLabel = this.createTextLabel(
      `${data.srG}`,
      new THREE.Vector3(data.srL / 2, -data.srH / 2 - 150, 0),
      0x000
    );
    if (dLabel) {
    ...
    // const dLine = this.createOuterDimensionLine(
    //   new THREE.Vector3(data.srL / 2 , data.srH / 2, data.srG / 2),
    //   new THREE.Vector3(data.srL / 2 + 200, data.srH / 2 + 200, data.srG / 2),
    //   offsetX,
    //   0x000
    // );
  ...
  private createSectionHDimension(data: Wardrobe, offset = 150) {
    ...
    if (data.SR_PLANKA_VERH_CHENTR || data.SR_PLANKA_VERH_LEV || data.SR_PLANKA_VERH_PRAV) {
      ...
      this.createDem(dimensionPlankaData, offset);
    }
  }
  ...
  private createDem(data: {
    ...
    z: number,
    x1: number
  }, offset = 150, color = 0xffffff, size = 200, isHorizontal = true) {
  ...
  hideDoors(visible: boolean) {
    ...
    this.scene.children.forEach(object => {
      if (object.name === 'EntresolList') {
        object.children.forEach(object => {
          if (object && object.name !== 'Entresol shelf') {
            object.visible = visible;
          }
        })
      }
